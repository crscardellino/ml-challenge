...
def remove_words(datasets, wordlist,
                 input_col_words="title", output_col_words="title",
                 input_col_pos="pos", output_col_pos="pos"):
    for split in tqdm(datasets, file=sys.stdout):
        token_pos = pd.Series(
            list(zip(datasets[split][input_col_words], datasets[split][input_col_pos]))
        )
        datasets[split][[output_col_words, output_col_pos]] = pd.DataFrame(
            token_pos.apply(
                lambda tp: list(
                    zip(*[
                        (tp[0][i], tp[1][i])
                        for i in range(len(tp[0]))
                        if tp[0][i] not in wordlist
                    ]))
            ).tolist()
        )
...
def pos_tags_to_idx(all_pos_tags, null_pos_tag="<NULL>", unknown_pos_tag="<UNK>"):
...
def pos_tag_sequence_padding(series, pos_tag_index, max_len):
    return pad_sequences(
            series.apply(
                lambda pos_tags: [pos_tag_index.get(pos_tag, pos_tag_index["<UNK>"])
...
def build_model(word_vocab_size, word_vector_size, word_embedding_matrix,
                ...
                pos_tag_dropout, pos_tag_vocab_size, pos_tag_vector_size,
    ...
    word_embedding_layer = Embedding(input_dim=word_vocab_size,
    ...
    pos_tag_embedded_sequences = Embedding(input_dim=pos_tag_vocab_size,
                                           output_dim=pos_tag_vector_size,
                                           embeddings_initializer="truncated_normal",
                                           trainable=True)(pos_tag_sequence_input)
    pos_tag_embedded_sequences = Dropout(
        rate=pos_tag_dropout,
        noise_shape=(None, 1, pos_tag_vector_size),
        seed=42
    ...
    for filter_len in word_filters_len:
        ...
        pos_layer = BatchNormalization(momentum=0.0)(pos_layer)
        pos_layers.append(GlobalMaxPooling1D()(pos_layer))

    layer = Concatenate()(layers + pos_layers)
    preds = Dense(output_size, activation="softmax")(layer)
    model = Model(inputs=[
        word_sequence_input,
        pos_tag_sequence_input,
        char_sequence_input], outputs=[preds])
...
def main(base_data_dir, language, output, activation, batch_size,
         ...
         pos_tag_dropout, pos_tag_vector_size, drop_columns, epochs, keep_punctuation,
         keep_stopwords, optimizer, padding, unreliable_sampling, use_normalized_char_tokens,
    ...
    if not keep_punctuation:
        ...
        datasets = remove_words(datasets, set(punctuation))

    if not keep_stopwords:
        logger.info("Removing stopwords from titles")
        datasets = remove_words(datasets, set(stopwords.words(language)))
    ...
    logger.info("Padding PoS tags sequences")
    train_pos_tag_sequences = pos_tag_sequence_padding(
        datasets["train"]["pos"], pos_tag_index, word_max_sequence_len
    )
    dev_pos_tag_sequences = pos_tag_sequence_padding(
        datasets["dev"]["pos"], pos_tag_index, word_max_sequence_len
    )
    test_pos_tag_sequences = pos_tag_sequence_padding(
        datasets["test"]["pos"], pos_tag_index, word_max_sequence_len
    )
    ...
    del test_pos_tag_sequences
    del test_char_sequences
    datasets["dev"] = datasets["dev"].loc[:, ["target"]]
    gc.collect()
    ...
    model = build_model(
        ...
        pos_tag_vocab_size=len(pos_tag_index),
        pos_tag_vector_size=pos_tag_vector_size,
    ...
    model.fit(
        (train_word_sequences, train_pos_tag_sequences, train_char_sequences), train_target,
        validation_data=(
    ...
    # eyeball_dataset[
    #     ["original_title", "normalized_tokens", "pos", "label_quality", "category", "pcategory"]
    # ].to_csv(
    ...
    datasets["test"]["predictions"] = model.predict(
        (test_word_sequences, test_pos_tag_sequences, test_char_sequences),
        batch_size=batch_size, verbose=0
    ).argmax(axis=1)
